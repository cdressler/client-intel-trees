...
async function withRetry<T>(fn: () => Promise<T>): Promise<T> {
  ...
  if (lastError instanceof APIConnectionError) {
    throw new AIProviderError('claude', 503, lastError.message);
  }
  if (lastError instanceof APIError) {
    throw new AIProviderError('claude', lastError.status ?? 500, lastError.message);
  }
  if (lastError instanceof Error) {
    throw new AIProviderError('claude', 500, lastError.message);
  }
  throw new AIProviderError('claude', 500, 'Unknown error communicating with Claude API');
}
...
export class ClaudeProvider implements AIProviderClient {
  ...
  async analyzeDocuments(documents: DocumentContent[]): Promise<DocumentInsights> {
    ...
    if (text.type !== 'text') {
      throw new AIProviderError('claude', 500, 'Unexpected response format from Claude');
    }
    return parseJsonResponse(text.text) as DocumentInsights;
  }
  ...
  async researchClient(clientName: string): Promise<ResearchFindings> {
    ...
    return parseJsonResponse(text.text) as ResearchFindings;
  }
...
Create root nodes that represent major conversation themes at the intersection of past work and current client intelligence. Under each root node, create leaf nodes with specific, actionable conversation starters — talking points or questions grounded in evidence from the documents and research. Every node must reference its source documents and/or research categories.`,
    ...
    return parseJsonResponse(text.text) as RawDecisionTree;
  }
}
